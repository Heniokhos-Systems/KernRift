...
> out of `ir.kr` into `codegen.kr` (`src/codegen.kr:838`–`640`).
...
`src/codegen.kr:608` makes the same point but quotes `116` for
...
fixup time, `fn_lookup()` (`src/codegen.kr:3306`) walks the function table
comparing with `tok_text_eq()` (`src/codegen.kr:1325`), which compares byte
...
why `builtin_override_tok_of()` (`src/codegen.kr:3384`) exists and returns
...
- **A provider reached only through override resolution gets pruned.** Under
  ...
  (`src/codegen.kr:13695`–`12035`). The seeds are deliberately *narrow* —
...
|---|---|---|---|
| x86_64 | `SYSCALL` (`0F 05`) | `emit_x86_syscall_insn` | `src/codegen.kr:699` |
...
(`src/codegen.kr:857`), beside `bare_metal_trap_refused`
(`src/codegen.kr:629`) — **in `codegen.kr`, not `ir.kr`**, precisely because
