...
(`src/ir.kr:5531`) from the AST and consumed by four machine-code emitters.
...
1. `IR_PHI` (opcode 60) is **completely dead**. Tree-wide, the only three
   ...
   in `ir_opcode_name` (`src/ir.kr:5296`), and a stale comment
...
is what makes that correct (`src/ir.kr:13162`–`13024`).
...
`ir_opcode_name()` (`src/ir.kr:5296`) is also **incomplete**: it has no entry
...
3. If it is an **emitter-level divergence**, remember that `ir_lower_stmt`
   (`src/ir.kr:4179`–`5147`) contains **no `target_os` branch at all** —
...
- Blocks print in index order, which is *also* the emission order
  (`src/ir.kr:13970`), but not the order the lowering created edges in: an
...
- **135/136/138/139 are optimizer products only.** No lowering emits them;
  ...
  (`src/ir.kr:12252`); 139's producer is gated to arch 0 and 1
  (`src/ir.kr:13286`).
...
- **`IR_FFMA` is a misnomer.** It emits `mulsd` + `addsd` on x86_64
  ...
  from a true FMA. The source says so at `src/ir.kr:13142`: *"The current
...
- **147/148 are arm64-only.** Produced solely by `ir_opt_fuse_lea_mem`, which
  is called under `if target_arch == 1` (`src/ir.kr:13198`). Handlers exist
...
- **`IR_BR_COND` does not "fall through".** The false target is the block's
  `succ0` field, read separately (`src/ir.kr:14029`). The emitter then picks
...
- Terminators are **not** emitted from the per-instruction dispatch on any
  backend. x86 has explicit no-op cases at `src/ir.kr:11849`/`11713` and
  skips them in the block loop (`src/ir.kr:13990`); arm64/riscv32/xtensa have
...
*not* in the side-effect set** (`ir_opt_is_side_effect`, `src/ir.kr:12509`,
...
(`src/ir.kr:12233`) and has handlers on all four backends. 140–142 and
144–145 are created only under `target_arch == 2` (`src/ir.kr:12437`), even
...
`src/ir.kr:12231` refuses to fold a shamt ≥ 32 when
...
|---|---|---|---|
| x86_64 | `ir_emit_x86_insn` | `src/ir.kr:8249` | `x86_nyi_op`, `src/ir.kr:8238` |
...
fixup time, `fn_lookup()` (`src/codegen.kr:2872`) walks the function table
...
why `builtin_override_tok_of()` (`src/codegen.kr:2950`) exists and returns
...
`IR_CALL` handler (`src/ir.kr:9951`–`9821`) records only an entry in the
...
`src/ir.kr:7927`; `ir_a64_syscall_nr` `src/ir_aarch64.kr:701`;
...
handler on all three IR backends that have one (`src/ir.kr:9820`,
...
|---|---|---|---|
| 52 `IR_SYSCALL` | `src/ir.kr:9809` | `src/ir_aarch64.kr:1624` | Windows → IAT thunks; macOS numbers get `0x2000000` OR'd; arm64 `open`→`openat` shift on Linux/Android but not macOS |
| 70 `IR_ALLOC` | `src/ir.kr:9149`, `9072` | `src/ir_aarch64.kr:1667`, `1690` | Windows → `VirtualAlloc(NULL, size+8, 0x3000, 4)` through the IAT; elsewhere inline `mmap` with flags `0x22` (Linux/Android) vs `0x1002` (macOS) |
| 71 `IR_DEALLOC` | `src/ir.kr:9149` | `1717` | `VirtualFree(ptr-8,0,MEM_RELEASE)` vs `munmap` |
...
| Op | Divergence |
|---|---|
| 126 `IR_ISB` | real `ISB` on arm64 (`src/ir_aarch64.kr:2734`); **complete no-op on x86_64**, zero bytes emitted (`src/ir.kr:10890`) |
| 128 `IR_ICACHE_INV` | `IC IVAU; DSB ISH; ISB` on arm64; **no-op on x86_64** (`src/ir.kr:10917`) |
...
**`ir_opt_is_side_effect(op)`** — `src/ir.kr:12509`. Used by DCE: an
...
**`ir_opt_cse_is_pure(op)`** — `src/ir.kr:12720`. Used by CSE to decide what
...
- **AST-level DCE seeds only `main` and `@export`** (`dce_scan`,
  `src/codegen.kr:12707`). `_start` is not a seed, so freestanding riscv32 and
...
- **A provider reached only through override resolution gets pruned.** Under
  ...
  (`src/codegen.kr:12542`–`12035`). The seeds are deliberately *narrow* —
...
The driver is `ir_optimize()` (`src/ir.kr:13144`). **There is no
...
`--O0` sets `ir_opt_level = 0` (`src/ir.kr:11907`, written only at
`src/main.kr:7647`) and each backend skips the whole call
(`src/ir.kr:13747`, `src/ir_aarch64.kr:3283`, `src/ir_riscv.kr:2414`,
...
`ir_licm_enabled` (`src/ir.kr:13130`) gates step 5 but is **never assigned
...
- LICM appends hoisted instructions at **high arena indices but low block
  ...
  they walk `ir_build_bb_lists` order (`src/ir.kr:12900`), not index ranges.
...
Loop metadata comes **only from the `while` lowering** (`src/ir.kr:4644`,
...
- **CSE-pure** (`src/ir.kr:12720`): 1–19, 20–27, 77, 79, 84, 86, 97–108,
  120–123, **and 143 only** of the immediate family.
- **Side-effecting** (`src/ir.kr:12509`): the list in §9.
...
`ir_graph_color()` (`src/ir.kr:6544`) is shared by all four backends through
a single colour→physical-register table (`ir_phys_regs`, sized once for the
largest file any arch uses, `src/ir.kr:6002`).
...
`src/ir.kr:6527` still says "5-bit set" — stale; it is 6 bits and includes
...
|---|---|---|---|---|
| x86_64 | 6 (`src/ir.kr:6011`) | rbx, r12, r13, r14, r15, **rbp** | **12** (`src/ir.kr:6048`) | + rsi, rdi, r8–r11 (all caller-saved) |
...
(`src/ir.kr:6035`–`5937`): **rax/rcx** are the universal spill-reload
...
`ir_x86_fn_wide_ok` (`src/ir.kr:6139`) / `ir_a64_fn_wide_ok`
(`src/ir_aarch64.kr:241`) test only the former; the call sites
(`src/ir.kr:13770`, `src/ir_aarch64.kr:3303`) add `&& is_naked == 0`. The asm
check must stay a **whole-body scan, not a constraint-list scan** — the
comment at `src/ir.kr:6115` records a real case where a CPUID block wrote r13
...
`ir_seed_wide_ceilings_generic` (`src/ir.kr:6194`) seeds these at
**instruction** granularity, not block granularity. The comment at
`src/ir.kr:6145` records why: sha256's whole 64-round compression body is one
...
Whitelists: `ir_x86_op_widesafe` (`src/ir.kr:6090`) and `ir_a64_op_widesafe`
...
`ir_color_ceil_to_reps` (`src/ir.kr:6402`) exists because coalescing colours
...
quietly landing in a caller-saved register. Called at `src/ir.kr:6966`, after
...
`ir_x86_seed_param_ceilings` (`src/ir.kr:6319`) cap every register-passed
...
Lives **inside** `ir_graph_color` (`src/ir.kr:6710`–`6843`), after the
interference graph is built and before the greedy loop. On by default;
`ir_coalesce_enabled` (`src/ir.kr:6459`) is read only in the loop condition
...
values: `ir_spill_count = ir_vreg_next - 1` (`src/ir.kr:13792`,
`src/ir_aarch64.kr:3319`). The comments at `src/ir.kr:7789` and
...
- **x86:** `[rsp + (vreg-1)*8]` (`ir_spill_offset`, `src/ir.kr:7788`);
...
- **Emit-time peepholes**, which are *not* part of `ir_optimize()` and are
  *not* disabled by `--O0`: a `store_spill r,v` immediately followed by
  ...
  (`src/ir.kr:7778`); the same vreg into a *different* register becomes a
...
**x86** — `ir_x86_xmm_alloc` (`src/ir.kr:7652`), homes `xmm2..xmm15`
(`xmm0`/`xmm1` are the float scratch pair and xmm0 is the SysV float return
register). It is called **unconditionally** (`src/ir.kr:7729`) with an
...
not: `ir_x86_xmm_mark_unsafe()` (`src/ir.kr:7525`) disqualifies, at
...
home instead of a stack slot (`src/ir.kr:7819`/`7756`). The one emitter that
reads slots directly — the >6-argument overflow path in `IR_CALL` — carries
an explicit home check at the site (`src/ir.kr:7343`). If you add an emitter
...
`src/ir.kr:3464`, `3461`, `3894`, `4164`, `4192`, `4209` emit `IR_ALLOC` with
...
7. **`IR_MODULE_PATH` (146) is missing from the side-effect set.**
   `ir_opt_is_side_effect` (`src/ir.kr:12509`) has no `op == 146` case even
...
9. **`--legacy --arch=arm64 --debug` emits no array bounds check.** Verified:
   ...
   IR arm64 does (`src/ir.kr:3620`/`4551` + trap at
...
11. **arm64 inline-asm register names are resolved with the x86 table.**
    `src/ir.kr:5088`/`5012` select `rv_reg_code` for riscv32 and
...
12. **Dead handler arms.** Xtensa's `IR_AND_IMM`/`OR_IMM`/`XOR_IMM`/
    ...
    unreachable — their only producer (`src/ir.kr:12437`) is gated to
...
15. `src/ir.kr:6527` — `ir_used_color_mask` says it "returns a 5-bit set …
...
16. `src/ir.kr:6717` — the coalescing section header says "aggressive … we
...
17. `src/ir.kr:6351` — says the colour-ceiling buffer is "only ever populated
...
2. Add the name to `ir_opcode_name()` (`src/ir.kr:5296`). It is currently
...
5. If it has a side effect, add it to `ir_opt_is_side_effect()`
   (`src/ir.kr:12509`). If it is safe to CSE, add it to
   `ir_opt_cse_is_pure()` (`src/ir.kr:12720`). These are two different
   decisions.
...
| | KernRift | MLRift |
|---|---|---|
| `--target=none` / bare metal | **yes** — 39 `target_os == 4` sites, four trap choke points, the `ir_bm_*` lowering layer (`src/ir.kr:1385`–`1538`) | **no** — zero `target_os == 4` sites anywhere. §12 does not apply |
...
| arm64 `IR_ADD_IMM`/`IR_SUB_IMM` (135/136) | handled (`src/ir_aarch64.kr:1258`) and produced (const-fold gate includes arch 1, `src/ir.kr:12286`) | **neither** — the const-folder excludes arm64 (`ir.mlr:11683`), so the emitter has no handler. Internally consistent, but a genuine gap |
| cmp-with-immediate fusion | arch 0 **and** 1 (`src/ir.kr:12182`, arm64 capped at imm12 4095) | arch 0 only (`ir.mlr:11590`) |
...
| per-BB instruction lists for colouring | flat lists via `ir_build_bb_lists` (`src/ir.kr:12900`) | a 65536-entry walk stack |
