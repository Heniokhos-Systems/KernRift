...
`target_os` values (parsed at `src/main.kr:9058`, derived from `--emit` at
...
- **A new arch × OS pair is validated by an allow-list**, not a blacklist:
  `arch_os_pair_supported()` (`src/main.kr:7514`) is per-row
...
`src/main.kr:9215`) and each backend skips the whole call
...
| `arch_os_pair_supported()` | yes (`src/main.kr:7514`) | **absent** — no arch × OS allow-list |
