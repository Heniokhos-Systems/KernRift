...
(`src/ir.kr:5742`) from the AST and consumed by four machine-code emitters.
...
1. `IR_PHI` (opcode 60) is **completely dead**. Tree-wide, the only three
   ...
   in `ir_opcode_name` (`src/ir.kr:5507`), and a stale comment
   (`src/ir_aarch64.kr:2240`). No lowering emits it and no emitter handles
   it.
...
"the vreg currently holding this variable" (`ir_var_get`, `src/ir.kr:1121`).
...
(`src/ir.kr:1157`), using a two-phase read-before-write ordering to avoid
...
32 bytes, fixed layout (accessors at `src/ir.kr:519`–`545`):
...
is what makes that correct (`src/ir.kr:13986`–`13024`).
...
(`src/ir.kr:622`); the block arena does **not** — exceeding 4096 blocks is a
hard `error: IR basic block overflow` and `exit(1)` (`src/ir.kr:720`).
...
> out of `ir.kr` into `codegen.kr` (`src/codegen.kr:891`–`640`).
...
`ir_opcode_name()` (`src/ir.kr:5507`) is also **incomplete**: it has no entry
...
`target_os` values (parsed at `src/main.kr:9098`, derived from `--emit` at
...
`src/codegen.kr:661` makes the same point but quotes `116` for
...
- **A new arch × OS pair is validated by an allow-list**, not a blacklist:
  `arch_os_pair_supported()` (`src/main.kr:7532`) is per-row
...
3. If it is an **emitter-level divergence**, remember that `ir_lower_stmt`
   (`src/ir.kr:4337`–`5147`) contains **no `target_os` branch at all** —
...
- Blocks print in index order, which is *also* the emission order
  (`src/ir.kr:15006`), but not the order the lowering created edges in: an
...
- **135/136/138/139 are optimizer products only.** No lowering emits them;
  ...
  (`src/ir.kr:13032`); 139's producer is gated to arch 0 and 1
  (`src/ir.kr:14066`).
...
- **`IR_FFMA` is a misnomer.** It emits `mulsd` + `addsd` on x86_64
  ...
  (`src/ir_aarch64.kr:2987`/`2871`). Two roundings, not one — results differ
  from a true FMA. The source says so at `src/ir.kr:13860`: *"The current
...
- **105/106 are not x86-only** (a claim the old text made). arm64 implements
  both as software bit manipulation, `src/ir_aarch64.kr:3016` and `2938`.
...
- **147/148 are arm64-only.** Produced solely by `ir_opt_fuse_lea_mem`, which
  is called under `if target_arch == 1` (`src/ir.kr:13986`). Handlers exist
...
- **`IR_BR_COND` does not "fall through".** The false target is the block's
  `succ0` field, read separately (`src/ir.kr:15126`). The emitter then picks
...
- Terminators are **not** emitted from the per-instruction dispatch on any
  backend. x86 has explicit no-op cases at `src/ir.kr:12629`/`11713` and
  skips them in the block loop (`src/ir.kr:15026`); arm64/riscv32/xtensa have
...
*not* in the side-effect set** (`ir_opt_is_side_effect`, `src/ir.kr:13289`,
...
(`src/ir.kr:13011`) and has handlers on all four backends. 140–142 and
144–145 are created only under `target_arch == 2` (`src/ir.kr:13278`), even
...
`src/ir.kr:13011` refuses to fold a shamt ≥ 32 when
...
|---|---|---|---|
| x86_64 | `ir_emit_x86_insn` | `src/ir.kr:8843` | `x86_nyi_op`, `src/ir.kr:8689` |
| AArch64 | `ir_emit_arm64_insn` | `src/ir_aarch64.kr:949` | `a64_nyi_op`, `src/ir_aarch64.kr:738` |
...
fixup time, `fn_lookup()` (`src/codegen.kr:3399`) walks the function table
comparing with `tok_text_eq()` (`src/codegen.kr:1378`), which compares byte
...
why `builtin_override_tok_of()` (`src/codegen.kr:3477`) exists and returns
...
`IR_CALL` handler (`src/ir.kr:10522`–`9821`) records only an entry in the
...
`src/ir.kr:8230`; `ir_a64_syscall_nr` `src/ir_aarch64.kr:755`;
...
**`ir_emit_syscall()` (`src/ir.kr:694`) is the only constructor of
`IR_SYSCALL`.** Verified: the only `ir_emit` call with `IR_SYSCALL` in the
tree is `src/ir.kr:695`, inside that function, and there is no `ir_emit(52,`
anywhere. It has 13 call sites, all in `src/ir.kr`. A future lowering
therefore cannot add an anonymous `IR_SYSCALL` by accident.

Readers: `ir_insn_origin_at()` (`src/ir.kr:511`), called from the op-52
handler on all three IR backends that have one (`src/ir.kr:10474`,
`src/ir_aarch64.kr:1723`, `src/ir_riscv.kr:2348`). Each falls back to
...
|---|---|---|---|
| 52 `IR_SYSCALL` | `src/ir.kr:10463` | `src/ir_aarch64.kr:1705` | Windows → IAT thunks; macOS numbers get `0x2000000` OR'd; arm64 `open`→`openat` shift on Linux/Android but not macOS |
| 70 `IR_ALLOC` | `src/ir.kr:9743`, `9072` | `src/ir_aarch64.kr:1748`, `1690` | Windows → `VirtualAlloc(NULL, size+8, 0x3000, 4)` through the IAT; elsewhere inline `mmap` with flags `0x22` (Linux/Android) vs `0x1002` (macOS) |
| 71 `IR_DEALLOC` | `src/ir.kr:9743` | `1717` | `VirtualFree(ptr-8,0,MEM_RELEASE)` vs `munmap` |
...
| 131 `IR_ARR_CHECK` | `10816` | `2744` | trap sequence differs per OS; **on arm64 + Windows the check is skipped entirely** (`if target_os == 2 { return }`, `src/ir_aarch64.kr:2863`) |
...
| Op | Divergence |
|---|---|
| 126 `IR_ISB` | real `ISB` on arm64 (`src/ir_aarch64.kr:2818`); **complete no-op on x86_64**, zero bytes emitted (`src/ir.kr:11670`) |
| 128 `IR_ICACHE_INV` | `IC IVAU; DSB ISH; ISB` on arm64; **no-op on x86_64** (`src/ir.kr:11697`) |
...
**`ir_opt_is_side_effect(op)`** — `src/ir.kr:13289`. Used by DCE: an
...
**`ir_opt_cse_is_pure(op)`** — `src/ir.kr:13500`. Used by CSE to decide what
...
- **AST-level DCE seeds only `main` and `@export`** (`dce_scan`,
  `src/codegen.kr:14305`). `_start` is not a seed, so freestanding riscv32 and
  xtensa entry points had to be added explicitly (`src/main.kr:2645`–`2374`).
- **A provider reached only through override resolution gets pruned.** Under
  ...
  (`src/codegen.kr:14103`–`12035`). The seeds are deliberately *narrow* —
...
The driver is `ir_optimize()` (`src/ir.kr:13932`). **There is no
...
`--O0` sets `ir_opt_level = 0` (`src/ir.kr:12687`, written only at
`src/main.kr:9257`) and each backend skips the whole call
(`src/ir.kr:14861`, `src/ir_aarch64.kr:3370`, `src/ir_riscv.kr:2414`,
...
`ir_licm_enabled` (`src/ir.kr:13910`) gates step 5 but is **never assigned
...
- LICM appends hoisted instructions at **high arena indices but low block
  ...
  they walk `ir_build_bb_lists` order (`src/ir.kr:13680`), not index ranges.
...
Loop metadata comes **only from the `while` lowering** (`src/ir.kr:4850`,
...
- **CSE-pure** (`src/ir.kr:13500`): 1–19, 20–27, 77, 79, 84, 86, 97–108,
  120–123, **and 143 only** of the immediate family.
- **Side-effecting** (`src/ir.kr:13289`): the list in §9.
...
`ir_graph_color()` (`src/ir.kr:6830`) is shared by all four backends through
a single colour→physical-register table (`ir_phys_regs`, sized once for the
largest file any arch uses, `src/ir.kr:6232`).
...
`src/ir.kr:6813` still says "5-bit set" — stale; it is 6 bits and includes
...
|---|---|---|---|---|
| x86_64 | 6 (`src/ir.kr:6241`) | rbx, r12, r13, r14, r15, **rbp** | **12** (`src/ir.kr:6287`) | + rsi, rdi, r8–r11 (all caller-saved) |
...
(`src/ir.kr:6265`–`5937`): **rax/rcx** are the universal spill-reload
...
`ir_x86_fn_wide_ok` (`src/ir.kr:6483`) / `ir_a64_fn_wide_ok`
(`src/ir_aarch64.kr:289`) test only the former; the call sites
(`src/ir.kr:14861`, `src/ir_aarch64.kr:3390`) add `&& is_naked == 0`. The asm
check must stay a **whole-body scan, not a constraint-list scan** — the
comment at `src/ir.kr:6398` records a real case where a CPUID block wrote r13
...
`ir_seed_wide_ceilings_generic` (`src/ir.kr:6477`) seeds these at
**instruction** granularity, not block granularity. The comment at
`src/ir.kr:6428` records why: sha256's whole 64-round compression body is one
...
Whitelists: `ir_x86_op_widesafe` (`src/ir.kr:6373`) and `ir_a64_op_widesafe`
...
`ir_color_ceil_to_reps` (`src/ir.kr:6685`) exists because coalescing colours
...
quietly landing in a caller-saved register. Called at `src/ir.kr:7278`, after
...
`ir_x86_seed_param_ceilings` (`src/ir.kr:6602`) cap every register-passed
...
Lives **inside** `ir_graph_color` (`src/ir.kr:7022`–`6843`), after the
interference graph is built and before the greedy loop. On by default;
`ir_coalesce_enabled` (`src/ir.kr:6745`) is read only in the loop condition
...
values: `ir_spill_count = ir_vreg_next - 1` (`src/ir.kr:14828`,
`src/ir_aarch64.kr:3412`). The comments at `src/ir.kr:8092` and
...
- **x86:** `[rsp + (vreg-1)*8]` (`ir_spill_offset`, `src/ir.kr:8091`);
...
- **arm64:** `IR_A64_OVERFLOW_RESERVE + (vreg-1)*8`
  ...
  (`src/ir_aarch64.kr:3471`), with 128 kept only as a floor for asm-block and
  Windows functions.
...
- **Emit-time peepholes**, which are *not* part of `ir_optimize()` and are
  *not* disabled by `--O0`: a `store_spill r,v` immediately followed by
  ...
  (`src/ir.kr:8043`); the same vreg into a *different* register becomes a
...
**x86** — `ir_x86_xmm_alloc` (`src/ir.kr:7955`), homes `xmm2..xmm15`
(`xmm0`/`xmm1` are the float scratch pair and xmm0 is the SysV float return
register). It is called **unconditionally** (`src/ir.kr:8028`) with an
...
not: `ir_x86_xmm_mark_unsafe()` (`src/ir.kr:7828`) disqualifies, at
...
home instead of a stack slot (`src/ir.kr:8122`/`7756`). The one emitter that
reads slots directly — the >6-argument overflow path in `IR_CALL` — carries
an explicit home check at the site (`src/ir.kr:7646`). If you add an emitter
...
|---|---|---|---|
| x86_64 | `SYSCALL` (`0F 05`) | `emit_x86_syscall_insn` | `src/codegen.kr:752` |
...
(`src/codegen.kr:910`), beside `bare_metal_trap_refused`
(`src/codegen.kr:682`) — **in `codegen.kr`, not `ir.kr`**, precisely because
...
`src/ir.kr:3630`, `3461`, `3894`, `4164`, `4192`, `4209` emit `IR_ALLOC` with
...
7. **`IR_MODULE_PATH` (146) is missing from the side-effect set.**
   `ir_opt_is_side_effect` (`src/ir.kr:13289`) has no `op == 146` case even
...
9. **`--legacy --arch=arm64 --debug` emits no array bounds check.** Verified:
   ...
   IR arm64 does (`src/ir.kr:3752`/`4551` + trap at
   `src/ir_aarch64.kr:2879`). `arr_count_lookup` is simply absent from
   `src/codegen_aarch64.kr`. Pre-existing, all targets.
10. **`IR_ARR_CHECK` is skipped entirely on arm64 + Windows**
    (`src/ir_aarch64.kr:2863`, `if target_os == 2 { return }`). Deliberate —
...
11. **arm64 inline-asm register names are resolved with the x86 table.**
    `src/ir.kr:5299`/`5012` select `rv_reg_code` for riscv32 and
...
12. **Dead handler arms.** Xtensa's `IR_AND_IMM`/`OR_IMM`/`XOR_IMM`/
    ...
    unreachable — their only producer (`src/ir.kr:13278`) is gated to
...
15. `src/ir.kr:6813` — `ir_used_color_mask` says it "returns a 5-bit set …
...
16. `src/ir.kr:7029` — the coalescing section header says "aggressive … we
...
17. `src/ir.kr:6634` — says the colour-ceiling buffer is "only ever populated
...
2. Add the name to `ir_opcode_name()` (`src/ir.kr:5507`). It is currently
...
5. If it has a side effect, add it to `ir_opt_is_side_effect()`
   (`src/ir.kr:13289`). If it is safe to CSE, add it to
   `ir_opt_cse_is_pure()` (`src/ir.kr:13500`). These are two different
   decisions.
...
| | KernRift | MLRift |
|---|---|---|
| `--target=none` / bare metal | **yes** — 39 `target_os == 4` sites, four trap choke points, the `ir_bm_*` lowering layer (`src/ir.kr:1454`–`1538`) | **no** — zero `target_os == 4` sites anywhere. §12 does not apply |
...
| arm64 `IR_ADD_IMM`/`IR_SUB_IMM` (135/136) | handled (`src/ir_aarch64.kr:1321`) and produced (const-fold gate includes arch 1, `src/ir.kr:13066`) | **neither** — the const-folder excludes arm64 (`ir.mlr:11683`), so the emitter has no handler. Internally consistent, but a genuine gap |
| cmp-with-immediate fusion | arch 0 **and** 1 (`src/ir.kr:12962`, arm64 capped at imm12 4095) | arch 0 only (`ir.mlr:11590`) |
...
| `arch_os_pair_supported()` | yes (`src/main.kr:7532`) | **absent** — no arch × OS allow-list |
| `--target=amdgpu-native` | no | yes |
| var map | FNV-1a open-addressed 4096-slot hash (`src/ir.kr:981`) | linear scan (`ir.mlr:1013`) |
| per-BB instruction lists for colouring | flat lists via `ir_build_bb_lists` (`src/ir.kr:13680`) | a 65536-entry walk stack |
