...
`target_os` values (assigned in `src/main.kr:7630`–`5258`, defaults at
...
- **A new arch × OS pair is validated by an allow-list**, not a blacklist:
  `arch_os_pair_supported()` (`src/main.kr:7126`) is per-row
...
- **AST-level DCE seeds only `main` and `@export`** (`dce_scan`,
  `src/codegen.kr:13095`). `_start` is not a seed, so freestanding riscv32 and
...
- **A provider reached only through override resolution gets pruned.** Under
  ...
  (`src/codegen.kr:12930`–`12035`). The seeds are deliberately *narrow* —
...
`src/main.kr:7773`) and each backend skips the whole call
...
| `arch_os_pair_supported()` | yes (`src/main.kr:7126`) | **absent** — no arch × OS allow-list |
