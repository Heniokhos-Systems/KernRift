...
`target_os` values (assigned in `src/main.kr:7505`–`5258`, defaults at
...
- **A new arch × OS pair is validated by an allow-list**, not a blacklist:
  `arch_os_pair_supported()` (`src/main.kr:7018`) is per-row
...
| `arch_os_pair_supported()` | yes (`src/main.kr:7018`) | **absent** — no arch × OS allow-list |
