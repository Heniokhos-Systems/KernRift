...
`target_os` values (assigned in `src/main.kr:7640`–`5258`, defaults at
...
- **A new arch × OS pair is validated by an allow-list**, not a blacklist:
  `arch_os_pair_supported()` (`src/main.kr:7136`) is per-row
...
`src/main.kr:7783`) and each backend skips the whole call
...
| `arch_os_pair_supported()` | yes (`src/main.kr:7136`) | **absent** — no arch × OS allow-list |
