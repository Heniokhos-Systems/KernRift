...
(`src/ir.kr:5643`) from the AST and consumed by four machine-code emitters.
...
1. `IR_PHI` (opcode 60) is **completely dead**. Tree-wide, the only three
   ...
   in `ir_opcode_name` (`src/ir.kr:5408`), and a stale comment
...
is what makes that correct (`src/ir.kr:13538`–`13024`).
...
`ir_opcode_name()` (`src/ir.kr:5408`) is also **incomplete**: it has no entry
...
3. If it is an **emitter-level divergence**, remember that `ir_lower_stmt`
   (`src/ir.kr:4258`–`5147`) contains **no `target_os` branch at all** —
...
- Blocks print in index order, which is *also* the emission order
  (`src/ir.kr:14554`), but not the order the lowering created edges in: an
...
- **135/136/138/139 are optimizer products only.** No lowering emits them;
  ...
  (`src/ir.kr:12592`); 139's producer is gated to arch 0 and 1
  (`src/ir.kr:13626`).
...
- **`IR_FFMA` is a misnomer.** It emits `mulsd` + `addsd` on x86_64
  ...
  from a true FMA. The source says so at `src/ir.kr:13482`: *"The current
...
- **147/148 are arm64-only.** Produced solely by `ir_opt_fuse_lea_mem`, which
  is called under `if target_arch == 1` (`src/ir.kr:13538`). Handlers exist
...
- **`IR_BR_COND` does not "fall through".** The false target is the block's
  `succ0` field, read separately (`src/ir.kr:14613`). The emitter then picks
...
- Terminators are **not** emitted from the per-instruction dispatch on any
  backend. x86 has explicit no-op cases at `src/ir.kr:12189`/`11713` and
  skips them in the block loop (`src/ir.kr:14574`); arm64/riscv32/xtensa have
...
*not* in the side-effect set** (`ir_opt_is_side_effect`, `src/ir.kr:12849`,
...
(`src/ir.kr:12573`) and has handlers on all four backends. 140–142 and
144–145 are created only under `target_arch == 2` (`src/ir.kr:12777`), even
...
`src/ir.kr:12571` refuses to fold a shamt ≥ 32 when
...
|---|---|---|---|
| x86_64 | `ir_emit_x86_insn` | `src/ir.kr:8566` | `x86_nyi_op`, `src/ir.kr:8555` |
...
`IR_CALL` handler (`src/ir.kr:10279`–`9821`) records only an entry in the
...
`src/ir.kr:8112`; `ir_a64_syscall_nr` `src/ir_aarch64.kr:755`;
...
handler on all three IR backends that have one (`src/ir.kr:10148`,
...
|---|---|---|---|
| 52 `IR_SYSCALL` | `src/ir.kr:10137` | `src/ir_aarch64.kr:1691` | Windows → IAT thunks; macOS numbers get `0x2000000` OR'd; arm64 `open`→`openat` shift on Linux/Android but not macOS |
| 70 `IR_ALLOC` | `src/ir.kr:9466`, `9072` | `src/ir_aarch64.kr:1734`, `1690` | Windows → `VirtualAlloc(NULL, size+8, 0x3000, 4)` through the IAT; elsewhere inline `mmap` with flags `0x22` (Linux/Android) vs `0x1002` (macOS) |
| 71 `IR_DEALLOC` | `src/ir.kr:9466` | `1717` | `VirtualFree(ptr-8,0,MEM_RELEASE)` vs `munmap` |
...
| Op | Divergence |
|---|---|
| 126 `IR_ISB` | real `ISB` on arm64 (`src/ir_aarch64.kr:2804`); **complete no-op on x86_64**, zero bytes emitted (`src/ir.kr:11230`) |
| 128 `IR_ICACHE_INV` | `IC IVAU; DSB ISH; ISB` on arm64; **no-op on x86_64** (`src/ir.kr:11257`) |
...
**`ir_opt_is_side_effect(op)`** — `src/ir.kr:12849`. Used by DCE: an
...
**`ir_opt_cse_is_pure(op)`** — `src/ir.kr:13060`. Used by CSE to decide what
...
The driver is `ir_optimize()` (`src/ir.kr:13484`). **There is no
...
`--O0` sets `ir_opt_level = 0` (`src/ir.kr:12247`, written only at
`src/main.kr:7773`) and each backend skips the whole call
(`src/ir.kr:14325`, `src/ir_aarch64.kr:3356`, `src/ir_riscv.kr:2414`,
...
`ir_licm_enabled` (`src/ir.kr:13470`) gates step 5 but is **never assigned
...
- LICM appends hoisted instructions at **high arena indices but low block
  ...
  they walk `ir_build_bb_lists` order (`src/ir.kr:13240`), not index ranges.
...
Loop metadata comes **only from the `while` lowering** (`src/ir.kr:4751`,
...
- **CSE-pure** (`src/ir.kr:13060`): 1–19, 20–27, 77, 79, 84, 86, 97–108,
  120–123, **and 143 only** of the immediate family.
- **Side-effecting** (`src/ir.kr:12849`): the list in §9.
...
`ir_graph_color()` (`src/ir.kr:6712`) is shared by all four backends through
a single colour→physical-register table (`ir_phys_regs`, sized once for the
largest file any arch uses, `src/ir.kr:6125`).
...
`src/ir.kr:6695` still says "5-bit set" — stale; it is 6 bits and includes
...
|---|---|---|---|---|
| x86_64 | 6 (`src/ir.kr:6134`) | rbx, r12, r13, r14, r15, **rbp** | **12** (`src/ir.kr:6171`) | + rsi, rdi, r8–r11 (all caller-saved) |
...
(`src/ir.kr:6158`–`5937`): **rax/rcx** are the universal spill-reload
...
`ir_x86_fn_wide_ok` (`src/ir.kr:6304`) / `ir_a64_fn_wide_ok`
(`src/ir_aarch64.kr:289`) test only the former; the call sites
(`src/ir.kr:14348`, `src/ir_aarch64.kr:3376`) add `&& is_naked == 0`. The asm
check must stay a **whole-body scan, not a constraint-list scan** — the
comment at `src/ir.kr:6280` records a real case where a CPUID block wrote r13
...
`ir_seed_wide_ceilings_generic` (`src/ir.kr:6359`) seeds these at
**instruction** granularity, not block granularity. The comment at
`src/ir.kr:6310` records why: sha256's whole 64-round compression body is one
...
Whitelists: `ir_x86_op_widesafe` (`src/ir.kr:6255`) and `ir_a64_op_widesafe`
...
`ir_color_ceil_to_reps` (`src/ir.kr:6567`) exists because coalescing colours
...
quietly landing in a caller-saved register. Called at `src/ir.kr:7151`, after
...
`ir_x86_seed_param_ceilings` (`src/ir.kr:6484`) cap every register-passed
...
Lives **inside** `ir_graph_color` (`src/ir.kr:6895`–`6843`), after the
interference graph is built and before the greedy loop. On by default;
`ir_coalesce_enabled` (`src/ir.kr:6627`) is read only in the loop condition
...
values: `ir_spill_count = ir_vreg_next - 1` (`src/ir.kr:14376`,
`src/ir_aarch64.kr:3398`). The comments at `src/ir.kr:7974` and
...
- **x86:** `[rsp + (vreg-1)*8]` (`ir_spill_offset`, `src/ir.kr:7973`);
...
- **Emit-time peepholes**, which are *not* part of `ir_optimize()` and are
  *not* disabled by `--O0`: a `store_spill r,v` immediately followed by
  ...
  (`src/ir.kr:7943`); the same vreg into a *different* register becomes a
...
**x86** — `ir_x86_xmm_alloc` (`src/ir.kr:7837`), homes `xmm2..xmm15`
(`xmm0`/`xmm1` are the float scratch pair and xmm0 is the SysV float return
register). It is called **unconditionally** (`src/ir.kr:7914`) with an
...
not: `ir_x86_xmm_mark_unsafe()` (`src/ir.kr:7710`) disqualifies, at
...
home instead of a stack slot (`src/ir.kr:8004`/`7756`). The one emitter that
reads slots directly — the >6-argument overflow path in `IR_CALL` — carries
an explicit home check at the site (`src/ir.kr:7528`). If you add an emitter
...
7. **`IR_MODULE_PATH` (146) is missing from the side-effect set.**
   `ir_opt_is_side_effect` (`src/ir.kr:12849`) has no `op == 146` case even
...
11. **arm64 inline-asm register names are resolved with the x86 table.**
    `src/ir.kr:5200`/`5012` select `rv_reg_code` for riscv32 and
...
12. **Dead handler arms.** Xtensa's `IR_AND_IMM`/`OR_IMM`/`XOR_IMM`/
    ...
    unreachable — their only producer (`src/ir.kr:12777`) is gated to
...
15. `src/ir.kr:6695` — `ir_used_color_mask` says it "returns a 5-bit set …
...
16. `src/ir.kr:6902` — the coalescing section header says "aggressive … we
...
17. `src/ir.kr:6516` — says the colour-ceiling buffer is "only ever populated
...
2. Add the name to `ir_opcode_name()` (`src/ir.kr:5408`). It is currently
...
5. If it has a side effect, add it to `ir_opt_is_side_effect()`
   (`src/ir.kr:12849`). If it is safe to CSE, add it to
   `ir_opt_cse_is_pure()` (`src/ir.kr:13060`). These are two different
   decisions.
...
| arm64 `IR_ADD_IMM`/`IR_SUB_IMM` (135/136) | handled (`src/ir_aarch64.kr:1312`) and produced (const-fold gate includes arch 1, `src/ir.kr:12626`) | **neither** — the const-folder excludes arm64 (`ir.mlr:11683`), so the emitter has no handler. Internally consistent, but a genuine gap |
| cmp-with-immediate fusion | arch 0 **and** 1 (`src/ir.kr:12522`, arm64 capped at imm12 4095) | arch 0 only (`ir.mlr:11590`) |
...
| per-BB instruction lists for colouring | flat lists via `ir_build_bb_lists` (`src/ir.kr:13240`) | a 65536-entry walk stack |
