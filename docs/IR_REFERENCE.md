...
`target_os` values (assigned in `src/main.kr:8795`–`5258`, defaults at
...
- **A new arch × OS pair is validated by an allow-list**, not a blacklist:
  `arch_os_pair_supported()` (`src/main.kr:7263`) is per-row
...
`src/main.kr:8952`) and each backend skips the whole call
...
| `arch_os_pair_supported()` | yes (`src/main.kr:7263`) | **absent** — no arch × OS allow-list |
