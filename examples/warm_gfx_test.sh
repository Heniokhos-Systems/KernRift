#!/usr/bin/env bash
# GFX ring doorbell bring-up — amdgpu-RESIDENT variant.
#
...
#   amdgpu(warm) -> unbind amdgpu -> bind mlrift_pci -> GFX ring doorbell
...
#              examples/kiq_doorbell_bringup.mlr -o /tmp/gfx_ring
...
LOG=${LOG:-/home/pantelis/mlrift_logs/warm_gfx_$(date +%Y%m%d-%H%M%S).log}
mkdir -p "$(dirname "$LOG")"
exec > >(tee -a "$LOG") 2>&1
echo "logging to $LOG"
say() { printf '\n=== %s ===\n' "$*"; }
