#!/usr/bin/env bash
# MES ucode bring-up (Task 0b — STATE-MUTATING) bring-up — amdgpu-RESIDENT variant.
#
...
#   amdgpu(warm) -> unbind amdgpu -> bind mlrift_pci -> MES ucode bring-up (Task 0b — STATE-MUTATING)
...
#              examples/mes_ucode_bringup.mlr -o /home/pantelis/mlrift_bin/mes_ucode_bringup
...
LOG=${LOG:-/home/pantelis/mlrift_logs/mes_ucode.log}
mkdir -p "$(dirname "$LOG")"
exec > >(tee -a "$LOG") 2>&1
echo "logging to $LOG"
say() { printf '\n=== %s ===\n' "$*"; }
...
restore() {
  ...
  say "restore: FLR + PCI remove + rescan (KIQ reprogrammed MMHUB; FLR resets it so amdgpu IP discovery can re-read)"
  sleep 1
...
# ---- Option 1: keep the GC power domain ON across the handoff ----
# DisallowGfxOff via SMU can't re-power a GFXOFF-gated dGPU (GC power-up is
# IMU-driven on-die, NOT an SMU message), so instead disable GFXOFF while amdgpu
# still owns the card, BEFORE unbind — then the GC domain never gates and the
# GFXHUB TLB-enable write should stick. debugfs polarity (amdgpu_debugfs.c doc):
# "write a 32-bit zero to disable" GFXOFF.
say "0) disable GFXOFF while amdgpu is bound (keep GC domain powered across handoff)"
DRI=""
for c in /sys/bus/pci/devices/$BDF/drm/card*; do
  [ -e "$c" ] && DRI="${c##*/card}" && break
done
if [ -n "$DRI" ]; then
  # The debugfs handler does get_user() of a u32 and rejects non-4-byte-aligned
  # writes with -EINVAL — so `echo 0` (2 ASCII bytes) FAILS. Write 4 zero bytes;
  # value 0 => amdgpu_gfx_off_ctrl(false) => disallow GFXOFF (done while amdgpu's
  # RLC is still live, so the GFXOFF-exit + IMU golden-RAM replay can complete).
  sudo sh -c "printf '\0\0\0\0' > /sys/kernel/debug/dri/$DRI/amdgpu_gfxoff" 2>/dev/null \
    && echo "  amdgpu_gfxoff disallowed (4-byte write) on dri/$DRI" \
    || echo "  WARN: could not disallow gfxoff on dri/$DRI — prefer the ppfeaturemask boot param"
else
  echo "  WARN: no drm/card* under $BDF — cannot locate the gfxoff knob"
fi
...
say "2) MES ucode bring-up (Task 0b — STATE-MUTATING)"
sudo /home/pantelis/mlrift_bin/mes_ucode_bringup
