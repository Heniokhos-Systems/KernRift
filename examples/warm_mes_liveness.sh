#!/usr/bin/env bash
# MES liveness probe (read-only) bring-up — amdgpu-RESIDENT variant.
#
...
#   amdgpu(warm) -> unbind amdgpu -> bind mlrift_pci -> MES liveness probe (read-only)
...
#              examples/mes_liveness_probe.mlr -o /home/pantelis/mlrift_bin/mes_liveness_probe
...
LOG=${LOG:-/home/pantelis/mlrift_logs/mes_liveness.log}
mkdir -p "$(dirname "$LOG")"
exec > >(tee -a "$LOG") 2>&1
echo "logging to $LOG"
say() { printf '\n=== %s ===\n' "$*"; }
...
say "2) MES liveness probe (read-only)"
sudo /home/pantelis/mlrift_bin/mes_liveness_probe
