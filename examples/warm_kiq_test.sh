#!/usr/bin/env bash
# KIQ + doorbell bring-up — amdgpu-RESIDENT variant.
#
...
#   amdgpu(warm) -> unbind amdgpu -> bind mlrift_pci -> KIQ + doorbell
...
#              examples/kiq_doorbell_bringup.mlr -o /home/pantelis/mlrift_bin/kiq_doorbell
...
LOG=${LOG:-/home/pantelis/mlrift_logs/warm_kiq_$(date +%Y%m%d-%H%M%S).log}
mkdir -p "$(dirname "$LOG")"
exec > >(tee -a "$LOG") 2>&1
echo "logging to $LOG"
say() { printf '\n=== %s ===\n' "$*"; }
...
say "2) KIQ + doorbell"
sudo /home/pantelis/mlrift_bin/kiq_doorbell
